...
- [ ] 1.9 Confirm the response shapes of `GET /org/{orgID}/roles`, `GET /org/{orgID}/clients`, and `GET /org/{orgID}/user-by-username`, and whether role and client names are unique. **Partially answered 2026-08-06.** Roles: `200`, `{"roles": [{"roleId", "name", "isAdmin", "description"}]}` -- `isAdmin` is `true` on the admin role and **absent** on others, and the two roles in the org (`Admin`, `Member`) have distinct names. Clients and `user-by-username` both return `403 "Key does not have permission perform this action"` for the key in `ingress/pangolin-api-key`, so their shapes and the uniqueness of client names are still unconfirmed, and client and user name resolution cannot work with this credential.
