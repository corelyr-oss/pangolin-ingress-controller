...
- [x] 2.2 Create `api/v1alpha1/groupversion_info.go` — group `pangolin.ingress.k8s.io`, version `v1alpha1`, `SchemeBuilder`, `AddToScheme`.
...
- [ ] 9.2 Install the CRD on a real cluster and confirm the CEL rules reject a `spec.public` block, a port entry with two forms, and an inverted range. **Not performed** — no cluster in this environment. The rules are present in the generated CRD (`deploy/crds/`); `helm template` renders the chart with and without `crds.install`.
