...
- [ ] 1.9 Confirm the response shapes of `GET /org/{orgID}/roles`, `GET /org/{orgID}/clients`, and `GET /org/{orgID}/user-by-username`, and whether role and client names are unique. **Mostly answered 2026-08-06; two gaps are environmental, not technical.**

  - **Roles** (`listRoles`): `200`, `{"roles": [...], "pagination": {"total", "page", "pageSize"}}`. A role carries `roleId`, `name`, `orgId`, `orgName`, `description`, `isAdmin`, `allowSsh`, `requireDeviceApproval` and four `ssh*` fields. `isAdmin` is `true` on the admin role and **absent** on others. The org's two roles have distinct names. Resolution verified end to end: an endpoint naming `Member` reconciles to `Ready=True`.
  - **Clients** (`listClients`): `200`, `{"clients": [...], "pagination": {...}}` -- same page/pageSize paging as roles, and **not** the limit/offset paging the site-resource listing uses. The org contains **no clients** (`total: 0`), so per-client field names and whether client names are unique could not be observed. Note this is despite a client being connected to the mesh, which is worth understanding on its own.
  - **User by username** (`getOrgUser`): the route and permission work, and the not-found path is confirmed -- an unknown username returns `404 "User with username '...' not found in organization"`, which the controller maps to `ResolvedRefs=False, PrincipalNotFound` with the message `user "...": not found in Pangolin`. The **success** shape is unobserved: no username is discoverable with the permissions granted (`listUsers`, `GET /org/{orgId}` and invitations are all `403`), so one has to be supplied.

  **Operational note:** granting `listClients` and `getOrgUser` initially *removed* `listRoles` -- roles returned `403` until it was granted again. Whatever edits API-key permissions appears to replace the set rather than add to it, so re-check the whole set after any change.
