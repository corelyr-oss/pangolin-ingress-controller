...
- [ ] 1.9 Confirm the response shapes of `GET /org/{orgID}/roles`, `GET /org/{orgID}/clients`, and `GET /org/{orgID}/user-by-username`, and whether role and client names are unique. **Roles answered 2026-08-06; clients and users still blocked.** Roles: `200`, `{"roles": [...], "pagination": {"total", "page", "pageSize"}}`; a role carries `roleId`, `name`, `orgId`, `orgName`, `description`, `isAdmin`, `allowSsh`, `requireDeviceApproval` and four `ssh*` fields. `isAdmin` is `true` on the admin role and **absent** on others. The two roles in the org have distinct names, and the response reports a `total`, so truncation is detectable here (the client's `listPaginated` does not read it, and stops on a short page instead -- adequate, but the signal exists). `GET /org/{orgID}/clients` and `GET /org/{orgID}/user-by-username` both still return `403 "Key does not have permission perform this action"` as of 2026-08-06, after role permission was granted. Their shapes, and whether client names are unique, remain unconfirmed; client and user name resolution cannot work with this key.
...
- [ ] 9.3 End-to-end against the live instance: create a `PangolinEndpoint`, verify the private resource in the Pangolin UI, reach it from a mesh client, mutate ports and principals, then delete and confirm cleanup. **All but the mesh reachability step done 2026-08-06:** create programmed the resource and reported `Ready=True` with a granted role; mutating ports to `8080`, `9000-9010` and UDP `5353` produced `tcp="8080,9000-9010" udp="5353"` server-side and the matching `.status.resolvedPorts`; removing the principals returned `Ready=False/NoPrincipalsGranted`; deletion removed the Pangolin resource and left the org with only its two pre-existing resources. **Reaching it from a mesh client is still not done** -- see task 1.3.
