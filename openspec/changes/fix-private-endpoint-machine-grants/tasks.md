...
- [ ] 6.3 Live: grant a machine client by nice ID against the real instance, confirm the resource shows it under Machines, and confirm the alias resolves from that client. Closes the client half of spike question 1.9 in `add-private-endpoint-crd/tasks.md`, which could not be answered when the org held no clients. **Not attempted** — deliberately deferred, out of the agreed scope for this change. Until it runs, two things stay assumptions: the client listing's field names (`clientId`/`niceId`/`name`), and whether Pangolin's create applies the grant arrays at all. The change is written so that the second no longer matters for correctness; the first still does, and a wrong field name would surface as `PrincipalNotFound` for every client.
